---
title: "Comparing model fits if we add in 2019 data"
author: "Mathew Kiang"
date: "11/24/2021"
...
## Introduction
Here, I will compare the **projections** from our paper (which only had data up to 2015) to the **observed** data for 2016 to 2019. See the [2016 version of this analysis](https://htmlpreview.github.io/?https://github.com/mkiang/opioid_trends/blob/main/rmds/add_analyses/add_2016_data.html) here ([code](https://github.com/mkiang/opioid_trends/blob/main/rmds/add_analyses/add_2016_data.Rmd)) or the [2017 version of this analysis](https://htmlpreview.github.io/?https://github.com/mkiang/opioid_trends/blob/main/rmds/add_analyses/add_2017_data.html) here ([code](https://github.com/mkiang/opioid_trends/blob/main/rmds/add_analyses/add_2017_data.Rmd)). 

We will compare two sets of joinpoint models:

1. Our published model that uses 1979 to 2015 (with a projection out to 2019)
1. The same model but with 1979 to 2019 data (i.e., all data and no projection)
...
## Comparing Figure 1
Now let's download and clean up both the 2015 and 2019 results.

```{r, message=FALSE, error=FALSE, warning=FALSE, results='hide'}
## Opioid mortality rates and joinpoint fits
opioid_mort <- bind_rows(
    import_url_results(2015, "rates_long", "data"), 
    import_url_results(2019, "rates_long", "data")
    ) %>% 
...
Then we take the 2015 results and project out to 2019.
```{r, cache=TRUE}
## Project out the 2015 results to 2019
opioid_mort <- bind_rows(
    opioid_mort %>% 
     ...
     opioid_mort %>% 
       ...
       opioid_mort %>% 
        filter(results_year == 2015, year == 2015) %>% 
        mutate(year = 2018, 
               model = (1 + as.numeric(gsub("^", "", apc, fixed = TRUE))/100)^3 * 
                   model, 
               std_rate = NA, 
               standarderror = NA),
       opioid_mort %>% 
        filter(results_year == 2015, year == 2015) %>% 
        mutate(year = 2019, 
               model = (1 + as.numeric(gsub("^", "", apc, fixed = TRUE))/100)^4 * 
                   model, 
               std_rate = NA, 
               standarderror = NA),
    opioid_mort
...
rate_ratios <- bind_rows(
    ...
    import_url_results(2019, "rate_ratio", "data")
    ) 
...
icd10_types <- bind_rows(
    ...
    import_url_results(2019, "icd10_type", "data")
) %>% 
    filter(race != "total", 
...
fig3 <- ggplot() + 
    ...
    facet_grid(opioid_cat ~ ., scales = "free") + 
    scale_color_brewer(NULL, palette = "Set1") + 
    scale_linetype_manual("Joinpoint Model Fit",
                          values = c("dashed", "solid")) +
    scale_x_continuous(NULL) + 
...
## Save the grobs
```{r}
saveRDS(fig1, "./../../output/add_analyses/fig1_with_2019.RDS")
saveRDS(fig3, "./../../output/add_analyses/fig3_with_2019.RDS")
```
